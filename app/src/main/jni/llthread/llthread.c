...
static llthread_t *llthread_create(lua_State *L, const char *code, size_t code_len) {
  ...
  lua_getglobal(child->L,"package");
  lua_getfield(L,-1,"cpath");
  lua_pushstring(child->L,lua_tostring(L,-1));
  lua_setfield(child->L,-1,"cpath");
  lua_pop(L,1);
  lua_getfield(L,-1,"path");
  lua_pushstring(child->L,lua_tostring(L,-1));
  lua_setfield(child->L,-1,"path");
  lua_pop(L,2);
  lua_pop(child->L,1);
