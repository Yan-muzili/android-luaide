...
typedef struct SearchResult {
    ...
    unsigned long value;    // 对应的值
    struct SearchResult* next;  // 指向下一个结果的指针
} SearchResult;

typedef SearchResult* PSearchResult;
...
// 监控线程函数声明
void* MemoryMonitorThread(void* arg);
...
system(log);//执行
if ((fp = fopen("android_log.txt", "r")) == NULL)
{
return -1;
}
fscanf(fp, "%d", &pid);//读取pid
remove("android_log.txt");
return pid;
}*/
...
PMAPS readmaps_a_anonmyous(PACKAGENAME *bm)
{
	...
	while (!feof(fp))
	{
		fgets(buff, sizeof(buff), fp); //读取一行
//		android_log(buff);
		if (strstr(buff, "rw") != NULL && !feof(fp) && (strlen(buff) < 50))
		{
			sscanf(buff, "%lx-%lx", &pNew->addr, &pNew->taddr);
			//这里使用lx是为了能成功读取特别长的地址
			flag = 1;
...
{
	FILE *fp;						 //文件指针
	system("date +%F-%T > android_log.txt"); //执行
	if ((fp = fopen("android_log.txt", "r")) == NULL)
	{
		return 0;
	}
	fscanf(fp, "%s", date); //读取
	remove("android_log.txt");		//删除
	return 1;
}
...
void* MemoryMonitorThread(void* arg) {
    ...
    if (pid == 0) {
        puts("Failed to get PID");
        return NULL;
    }

    while (monitor_running) {
        // 修改 pMap 类型为 PMAPS
        PMAPS pMap = readmaps_all(packagename);
        if (pMap != NULL) {
            // 加锁保护监控结果
            pthread_mutex_lock(&results_mutex);
            // 清空之前的监控结果
            if (monitor_results != NULL) {
                PMAPS tmp = reinterpret_cast<PMAPS>(monitor_results);
                while (tmp != NULL) {
                    PMAPS next = tmp->next;
                    free(tmp);
                    tmp = next;
...
// 搜索内存中的值，返回结果链表
PSearchResult searchMemoryForValue(char *bm, unsigned long value, size_t valueSize, unsigned long startAddress, unsigned long endAddress) {
    PSearchResult resultHead = NULL;
    PSearchResult resultTail = NULL;

    // 读取内存映射
    PMAPS maps = readmaps_all(bm);
    if (!maps) {
        return NULL;
    }
    ...
    // 打开进程的内存文件
    char memPath[256];
    sprintf(memPath, "/proc/%d/mem", getPID(bm));
    ...
    // 遍历每个内存区域
    PMAPS currentMap = maps;
    while (currentMap) {
        // 计算内存区域大小
        size_t regionSize = currentMap->taddr - currentMap->addr;

        // 检查是否在指定的内存区域内
        if (currentMap->addr >= startAddress && currentMap->taddr <= endAddress) {
            // 分配缓冲区读取内存内容
            ...
            // 定位到内存区域开始位置
            if (fseek(memFile, currentMap->addr, SEEK_SET) != 0) {
                free(buffer);
                currentMap = currentMap->next;
                continue;
            }
            ...
            // 读取内存内容
            size_t bytesRead = fread(buffer, 1, regionSize, memFile);
            ...
            for (size_t i = 0; i <= bytesRead - valueSize; i++) {
                ...
                for (size_t j = 0; j < valueSize; j++) {
                    if (((unsigned char*)&value)[j] != buffer[i + j]) {
                        match = 0;
                        break;
                    }
                }
                ...
                // 如果找到匹配，添加到结果链表
                if (match) {
                    PSearchResult newResult = (PSearchResult)malloc(sizeof(SearchResult));
                    if (newResult) {
                        newResult->address = currentMap->addr + i;
                        newResult->value = value;
                        newResult->next = NULL;

                        if (!resultHead) {
                            resultHead = resultTail = newResult;
                        } else {
