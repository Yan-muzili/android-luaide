...
public class TextStyle {
    ...
    /**
     * 标志位：如果置位，表示 background 参数是原始 ARGB，而不是颜色 ID
     */
    public static final long USE_ARGB_BG = 1L << 63;   // 用最高位，不会与现有位冲突

    /**
     * 新增重载
     *
     * @param background  背景值。如果 useArgb = true，则为 ARGB；否则为颜色 ID
     * @param useArgb     是否把 background 当 ARGB 使用
     */
    public static long makeStyle(int foregroundColorId,
                                 int background,
                                 boolean bold,
        ...
        long style = ((long) foregroundColorId)
                ...
                | (noCompletion ? NO_COMPLETION_BIT : 0);
        if (useArgb) {
            style |= USE_ARGB_BG;   // 打上“这是 ARGB”标记
        }
        return style;
    }
