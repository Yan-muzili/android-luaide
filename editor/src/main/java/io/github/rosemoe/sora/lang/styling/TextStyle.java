 ...
 *    https://github.com/Rosemoe/sora-editor GitHub - Rosemoe/sora-editor: A multifunctional Android code editor library. (aka CodeEditor) GitHub - Rosemoe/sora-editor: A multifunctional Android code editor library. (aka CodeEditor)
...
public class TextStyle {
    ...
    public static final long USE_ARGB_BG = 1L << 63;

    /**
     * 新重载：允许直接传 ARGB
     * useArgb=true -> background 就是 ARGB
     */
    public static long makeStyle(int foregroundColorId,
                                 int background,
                                 boolean bold,
                                 ...
                                 boolean useArgb) {
        checkColorId(foregroundColorId);
        long style = ((long) foregroundColorId)
                | (((long) (background & 0xFFFFFFFFL)) << COLOR_ID_BIT_COUNT)
                | (bold ? BOLD_BIT : 0)
                | (italic ? ITALICS_BIT : 0)
                | (strikeThrough ? STRIKETHROUGH_BIT : 0)
                | (noCompletion ? NO_COMPLETION_BIT : 0);
        if (useArgb) style |= USE_ARGB_BG;
        return style;
    }

    public static int getBackgroundColor(long style, @NonNull EditorColorScheme scheme) {
        if ((style & USE_ARGB_BG) != 0) {
            return (int) ((style >>> COLOR_ID_BIT_COUNT) & 0xFFFFFFFFL);
        } else {
            int id = (int) ((style >>> COLOR_ID_BIT_COUNT) & ((1 << COLOR_ID_BIT_COUNT) - 1));
