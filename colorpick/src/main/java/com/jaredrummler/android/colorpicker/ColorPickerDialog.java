 ...
 *
 * <p>The {@link Activity activity} that shows this dialog should implement {@link
...
public class ColorPickerDialog extends DialogFragment
  ...
  public AlertDialog.Builder MdDialogBuilder;

  // -- CUSTOM ---------------------------
  ColorPickerView colorPicker;
  ColorPanelView newColorPanel;
  TextInputLayout hexLayout;
  TextInputLayout rgbalayout;
  TextInputLayout hslalayout;
  EditText hexEditText;
  EditText rgbaEditText;
  EditText hslaEditText;
  public String MODE = "Hex";
  boolean showAlphaSlider;
  private int presetsButtonStringRes;
  private boolean fromEditText;
  private int customButtonStringRes;

  private final OnTouchListener onPickerTouchListener =
      new OnTouchListener() {
        @Override
  ...
  public Dialog onCreateDialog(Bundle savedInstanceState) {
    ...
    /*
        builder.setNegativeButton("图库", (dialog,which)->{
        //图库
        Intent intent = new Intent(Intent.ACTION_GET_CONTENT);
        intent.addCategory(Intent.CATEGORY_OPENABLE);
        intent.setType("image/*");
        startActivityForResult(intent, 3);
        // progressDialog = fileCtrl.showProgressDialog("导入", "正在打开系统文件选择界面...");
        });
    */

    this.MdDialogBuilder = builder;
   ...
   * Set the callback.
   *
   * <p>Note: The preferred way to handle the callback is to have the calling Activity implement
   * {@link ColorPickerDialogListener} as this will not survive an orientation change.
   *
   * @param colorPickerDialogListener The callback invoked when a color is selected or the dialog is
   *     dismissed.
   */
  ...
  View createPickerView() {
    ...
    ColorPanelView oldColorPanel =
        (ColorPanelView) contentView.findViewById(R.id.cpv_color_panel_old);
    ...
    hexLayout = contentView.findViewById(R.id.cpv_hex_layout);
    hexEditText = (EditText) contentView.findViewById(R.id.cpv_hex);

    // 新增其它颜色代码
    rgbalayout = contentView.findViewById(R.id.cpv_rgba_layout);
    rgbaEditText = (EditText) contentView.findViewById(R.id.cpv_rgba);

    hslalayout = contentView.findViewById(R.id.cpv_hsla_layout);
    hslaEditText = (EditText) contentView.findViewById(R.id.cpv_hsla);

    ChipGroup chipGroup = contentView.findViewById(R.id.myChipGroup);
    Chip chipHex = contentView.findViewById(R.id.hex);
    Chip chipRGBA = contentView.findViewById(R.id.rgba);
    Chip chipHSLA = contentView.findViewById(R.id.hsla);
    ...
    chipGroup.setOnCheckedChangeListener(
        new ChipGroup.OnCheckedChangeListener() {
          ...
          public void onCheckedChanged(ChipGroup group, int checkedId) {
            ...
            rgbalayout.setVisibility(View.GONE);
            hslalayout.setVisibility(View.GONE);

            Chip checkedChip = group.findViewById(checkedId);

            if (checkedChip == chipHex) {
              hexLayout.setVisibility(View.VISIBLE);
              MODE = "Hex";
              hexEditText.requestFocus();
              hexEditText.setSelection(hexEditText.getText().length());
            }
            if (checkedChip == chipRGBA) {
              rgbalayout.setVisibility(View.VISIBLE);
              rgbaEditText.requestFocus();
              MODE = "RGBA";
              rgbaEditText.setSelection(rgbaEditText.getText().length());
            }

            if (checkedChip == chipHSLA) {
              hslalayout.setVisibility(View.VISIBLE);
              hslaEditText.requestFocus();
              hslaEditText.setSelection(hslaEditText.getText().length());
            }
          }
        });

    if (getArguments().getString("MODE").equals("RGBA")) {
      chipRGBA.setChecked(true);
    }
    ...
    try {
      ...
      TypedArray typedArray =
          getActivity()
              .obtainStyledAttributes(value.data, new int[] {android.R.attr.textColorPrimary});
    ...
    rgbaEditText.addTextChangedListener(
        new TextWatcher() {
          @Override
          ...
          public void afterTextChanged(Editable s) {
            if (hexLayout.getVisibility() == View.GONE) {
              int color = MyColorUtils.rgbaStringToInt(s.toString());
              if (color != colorPicker.getColor()) { // 非原颜色则改变画板颜色
                fromEditText = true;
  ...
  public void onColorChanged(int newColor) {
    ...
    if (!fromEditText && hexEditText != null) {
      ...
      if (rgbaEditText.hasFocus()) {
        InputMethodManager imm =
                (InputMethodManager) getActivity().getSystemService(Context.INPUT_METHOD_SERVICE);
        imm.hideSoftInputFromWindow(rgbaEditText.getWindowToken(), 0);
        rgbaEditText.clearFocus();
      }
    }
    fromEditText = false;
  }
  ...
  private void setHex(int color) {
    rgbaEditText.setText(MyColorUtils.intToRgbaString(color));
    if (showAlphaSlider) {
      hexEditText.setText(String.format("%08X", (color)));
    } else {
  ...
  View createPresetsView() {
    ...
    adapter =
        new ColorPaletteAdapter(
            new ColorPaletteAdapter.OnColorSelectedListener() {
              @Override
  ...
  private void loadPresets() {
    ...
    presets =
        Arrays.copyOf(
            presets, presets.length); // don't update the original array when modifying alpha
    if (alpha != 255) {
    ...
    if (initialColor != color) {
      // The user clicked a color and a configuration change occurred. Make sure the initial color
      // is in the presets
      presets = unshiftIfNotExists(presets, initialColor);
    }
    if (isMaterialColors && presets.length == 19) {
      // Add black to have a total of 20 colors if the current color is in the material color
      // palette
  ...
  void createColorShades(@ColorInt final int color) {
    ...
    final int horizontalPadding =
        getResources().getDimensionPixelSize(R.dimen.cpv_item_horizontal_padding);
    ...
    for (final int colorShade : colorShades) {
      ...
      ViewGroup.MarginLayoutParams params =
          (ViewGroup.MarginLayoutParams) colorPanelView.getLayoutParams();
          ...
          new View.OnClickListener() {
            ...
            public void onClick(View v) {
              ...
              for (int i = 0; i < shadesLayout.getChildCount(); i++) {
                FrameLayout layout = (FrameLayout) shadesLayout.getChildAt(i);
                ColorPanelView cpv =
                    (ColorPanelView) layout.findViewById(R.id.cpv_color_panel_view);
                ImageView iv = (ImageView) layout.findViewById(R.id.cpv_color_image_view);
  ...
  private int[] getColorShades(@ColorInt int color) {
    return new int[] {
      shadeColor(color, 0.9), shadeColor(color, 0.7), shadeColor(color, 0.5),
          shadeColor(color, 0.333),
      shadeColor(color, 0.166), shadeColor(color, -0.125), shadeColor(color, -0.25),
          shadeColor(color, -0.375),
      shadeColor(color, -0.5), shadeColor(color, -0.675), shadeColor(color, -0.7),
          ...
          public void onProgressChanged(SeekBar seekBar, int progress, boolean fromUser) {
            ...
            for (int i = 0; i < shadesLayout.getChildCount(); i++) {
              ...
              if (cpv.getTag() != null && (Boolean) cpv.getTag()) {
                // The alpha changed on the selected shaded color. Update the checkmark color
                // filter.
                if (alpha <= ALPHA_THRESHOLD) {
                  iv.setColorFilter(Color.BLACK, PorterDuff.Mode.SRC_IN);
                } else {
  ...
  public static final class Builder {
     ...
     *
     * @param dialogType Either {@link ColorPickerDialog#TYPE_CUSTOM} or {@link
    ...
    public Builder setColor(String color) {
      if (color.toLowerCase().startsWith("rgb")) {
        setMODE("RGBA");
        this.color = MyColorUtils.rgbaStringToInt(color);
      } else {
        setMODE("Hex");
      try {
     ...
     *
     * @param dialogId The id that is sent whiteBack to the {@link ColorPickerDialogListener}.
     * @return This builder object for chaining method calls
     */
